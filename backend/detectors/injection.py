...
# Each pattern targets a concrete manipulation phrase. Kept narrow so ordinary
# messages (which never try to reprogram an analyzer) don't match.
_PATTERNS = [
...
def analyze_injection(text: str) -> list[Signal]:
    ...
    return [Signal(
        ...
        detail=("The message tries to instruct the analyzer itself — for example to "
